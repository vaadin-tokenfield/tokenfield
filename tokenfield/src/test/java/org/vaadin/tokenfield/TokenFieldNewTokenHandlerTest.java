...
class TokenFieldNewTokenHandlerTest {
    ...
    // ------------------------------------------------------------------
    // Default handler: onTokenInput first, then the container (back-compat)
    // ------------------------------------------------------------------

    @Test
    void theDefaultHandlerCallsOnTokenInputBeforeRememberingTheToken() {
        ...
        assertWithMessage("Pre-#39 order kept for backward compatibility")
                .that(inContainerOnInput).containsExactly(false);
        assertThat(observing.getValue()).containsExactly("tag1");
        assertThat(observing.getComboBox().containsId("tag1")).isTrue();
    }

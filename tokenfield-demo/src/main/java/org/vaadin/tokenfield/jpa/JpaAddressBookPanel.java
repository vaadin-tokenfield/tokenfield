 ...
 * add-on reads such a refusal as "no such item" and names an off-book token
 * after itself, so nothing propagates. {@link #getTokenCaption(Object)} still
 * answers for those tokens without asking the container, which spares it the
 * failed conversion and the query behind it.
 * </p>
 */

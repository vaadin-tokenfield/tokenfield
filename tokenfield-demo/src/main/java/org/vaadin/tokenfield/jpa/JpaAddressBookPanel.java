 ...
 * add-on no longer walks into it: a container that refuses an id counts as one
 * that does not hold it, so the caption falls back to the id. The
 * {@link #getTokenCaption(Object)} override below is kept because this panel
 * wants a specific answer for an off-book token, not because the add-on needs
 * the help.
 * </p>
 */

...
- Typed text that matches no suggestion goes to the `NewTokenHandler` (`setNewTokenHandler`,
  mirrors `ComboBox.setNewItemHandler`). `DefaultNewTokenHandler` does `onTokenInput(text)` and
  then `rememberToken` (pre-#39 order, kept for 7.0.x backward compatibility). A container that
  assigns its own ids needs a custom handler; the value stays the source of truth, the container
  supplies suggestions and captions, and a token the container does not hold is a supported case.
